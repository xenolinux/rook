...
func removeOSD(context *clusterd.Context, clusterInfo *client.ClusterInfo, osdID int) error {
	...
	prepareJobName := fmt.Sprintf("rook-ceph-osd-prepare-example-deviceset-0-#{osdID}")
	prepareJob, err := context.Clientset.BatchV1().Jobs(clusterInfo.Namespace).Get(prepareJobName, metav1.GetOptions{});
	// Delete the osd prepare job
	logger.Infof("removing the OSD prepare job %q", prepareJobName)
	if err := k8sutil.DeleteBatchJob(context.Clientset, clusterInfo.Namespace, prepareJobName, false); err != nil {
		if !kerrors.IsNotFound(err) {
			// Continue deleting the OSD prepare job even if the deployment fails to be deleted
			logger.Errorf("failed to delete prepare job for OSD %s. %v", prepareJobName, err)
		}
	}

	// Check if OSD is running on PVC
	if pvcName, ispresent := prepareJob.GetLabels()["ceph.rook.io/pvc"];ispresent{
		// If running on a PVC, delete the OSD PVC
		...
		if err := context.Clientset.CoreV1().PersistentVolumeClaims(clusterInfo.Namespace).Delete(pvcName, &metav1.DeleteOptions{}); err != nil {
			if !kerrors.IsNotFound(err) {
				...
				logger.Errorf("failed to delete pvc for OSD %s. %v", pvcName, err)
			}
		}
	}
